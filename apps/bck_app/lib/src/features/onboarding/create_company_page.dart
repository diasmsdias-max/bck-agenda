  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final created = await widget.apiClient.createCompany(BootstrapCompanyRequest(
        ...
        ownerUsername: _username.text.trim(),
      ...
      Navigator.of(context).pushAndRemoveUntil(
        MaterialPageRoute(builder: (_) => _CreatedPage(companyName: created.companyName, ownerName: session.name)),
        (_) => false,
      );
    } catch (_) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Não foi possível criar a empresa. Verifique a conexão e os dados informados.')));
      }
    } finally {
          ...
          child: Center(
            child: Padding(
              ...
              child: Column(
                ...
                children: [
                  const Icon(Icons.check_circle_rounded, size: 72, color: Color(0xFFD6A84B)),
                  const SizedBox(height: 20),
                  Text('Empresa criada', style: Theme.of(context).textTheme.headlineSmall?.copyWith(fontWeight: FontWeight.w700)),
                  const SizedBox(height: 12),
                  Text('$companyName está conectado.\nBem-vindo, $ownerName.', textAlign: TextAlign.center),
                  const SizedBox(height: 20),
                  const Text('Sua sessão foi protegida neste aparelho.', textAlign: TextAlign.center, style: TextStyle(color: Colors.white70)),
                ],
              ),
            ),
          ),
