...
class ServiceSessionPage extends StatefulWidget {
  ...
  @override State<ServiceSessionPage> createState() => _ServiceSessionPageState();
}
...
class _ServiceSessionPageState extends State<ServiceSessionPage> {
  ...
  Future<T?> _busy<T>(Future<T> Function() action) async {
    ...
    try { return await action(); } catch (error) { if (mounted) setState(() => _error = error); return null; }
  ...
  Future<void> _finish() async {
    ...
    final confirmed = await showDialog<bool>(context: context, builder: (context) => AlertDialog(
      ...
      actions: [TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Voltar')), FilledButton(onPressed: () => Navigator.pop(context, true), child: const Text('Finalizar'))],
    ));
