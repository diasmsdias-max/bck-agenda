...
class _ServiceSessionPageState extends State<ServiceSessionPage> {
  ...
  Future<void> _openOrResume() async {
    ...
    try {
      ...
      final opened = await widget.api.open(
  ...
  Future<void> _refresh() async {
    ...
    final value = await _busy(() => widget.api.get(session.id));
  ...
  Future<void> _addItem() async {
    final session = _session;
    if (session == null) return;
    final draft = await showDialog<ServiceSessionItemDraft>(
      context: context,
      builder: (_) => const AddServiceSessionItemDialog(),
    );
    if (draft == null) return;
    final item = await _busy(
      () => widget.api.addItem(
        session.id,
        itemType: 'PRODUCT',
        name: draft.name,
  ...
  Future<void> _finish() async {
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Text(
          'Total preparado para recebimento: ${_money(session.total)}',
        ),
        actions: [
          TextButton(
      ...
      child: Scaffold(
        ...
        body: ListView(
          ...
          children: [
            ...
            Text(
              'Agendamento ${widget.appointmentId}',
            ...
            if (session == null)
              FilledButton.icon(
                onPressed: _loading ? null : _openOrResume,
                icon: const Icon(Icons.play_arrow_rounded),
                label: Text(
                  _loading
                      ? 'Carregando atendimento...'
                      : 'Tentar carregar atendimento novamente',
                ),
              )
            else ...[
              Card(
              ...
              if (!session.isClosed) ...[
                FilledButton.tonalIcon(
                  ...
                  label: const Text('Adicionar produto/item extra'),
                ),
                ...
                const Card(
                  child: Padding(
                    ...
                    child: Text(
                      'Atendimento finalizado e preparado para recebimento.',
                    ),
                  ),
                ),
            ],
